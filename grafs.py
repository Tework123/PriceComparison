...
# подгоняет время под нужный формат
def get_time(time):
...
# возвращает график matplotlib
async def get_graf(list_times):

    # сортировка первичного списка для последущей укладки значений в график
    ...
    # укладываем отсортированные значения в график
    for one_list in result:

        # еще укладываем текст для сообщения, которое потом отправиться юзеру
        text_for_message.append((f'{one_list[-1][0]}, '
                                 f' Цена без скидки: {one_list[-1][1]}, '
                                 f' Цена со скидкой: {one_list[-1][2]}, '
                                 f' Ссылка: {one_list[-1][3]}, '
                                 f' Желаемая цена: {one_list[-1][5]}'))
    ...
    # подгоняет временную шкалу графика под определенные значения, в зависимости от кол-ва спаршенного
    if len(result[0]) < 10:
